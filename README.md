...
游戏共 **10 波** PvE 回合，击败全部 10 波即为通关。每回合分为两个阶段：
...
第 10 波 Boss 战还会出现特殊 Boss 单位（恶魔/Boss 标签，2★ 属性）。
...
- 每回合胜利获得金币奖励（随波数递增）
- 失败获得 **5 金币** 补偿，保证后续还能玩
- 出售单位返还金币：费用 × 星级倍率（1★×1、2★×3、3★×9）
- 金币用于购买单位和刷新商店（刷新消耗 2 金币）

### 合星系统

- **3 个同名 1★ 单位 → 1 个 2★ 单位**（属性大幅提升）
- **2 个同名 2★ + 1 个同名 1★ → 1 个 3★ 单位**（属性再次提升）
- 准备阶段开始时会自动检测并合星
- 高星级单位战斗力远超低星级，合理合星是取胜关键

### 羁绊系统

每个单位拥有**羁绊标签**（如"人类"、"战士"、"精灵"、"法师"等）。场上同名羁绊数量达到阈值时激活队伍加成：

- **≥2 个同羁绊单位** → 激活一档
- **≥4 个同羁绊单位** → 激活二档（效果更强）

例如：上阵 2 个人类单位即可激活「人类」一档羁绊，上阵 4 个则激活二档。合理搭配不同羁绊可以大幅提升阵容强度。
